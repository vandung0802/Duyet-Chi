...
>
> **Cập nhật lần cuối:** phiên bản app **v130** (`APP_VERSION = '20260803-v130'`, `sw.js = 20260803-130`, `version.txt = 20260803-v130`) — **ĐÃ PHÁT HÀNH cho cả công ty**. Chuỗi mới nhất: **v130** (ấn đúp bảng thống kê P.KH "in đúc" báo cáo kế toán + hết toast 2 lần — mục BQ), **v129** (ấn đúp dòng công trình thống kê P.KH — gộp vào BQ), **v128** (đổi thứ tự nav: Danh sách·Thêm mới·Báo cáo·Vật tư·P.KH·Lương·Cài đặt — mục BQ), **v125–v127** (dự án P.Kế hoạch: tab P.KH kind='kh' kho riêng khProposals, chat 2 kênh, siết luật kín 2 chiều, sheet "Chi Kế hoạch", Apps Script v30 — mục BQ; kèm sửa lỗi nền _cardHash + renderAll quên page-kh), **v124** (trường phòng dept + sửa tên + quy trình deploy ÂM THẦM — mục BQ). Trước đó **v119–v123** (sheet Google tự tô đẹp + thống kê dashboard đúng + nút có màu — mục BP), **v118** (sheet "Tổng hợp Lương" — mục BO). Trước đó **v117** (tin ghim đổi màu đỏ — mục BN), **v116** (menu tin nhắn kiểu Zalo + @Tất cả + ghim + trả lời — mục BM), **v115** (ảnh trong Chat nhóm — mục BL), **v114** (3 bảng giãn hết màn hình máy tính — mục BK), **v113** (xóa/thu hồi tin nhắn chat — mục BJ), **v112** (bảng nhắc 2 nút Để sau/Xong — mục BI), **v111** (4 chỉnh chat — mục BH), **v110** (hết trùng nội dung ý kiến — mục BG), **v109** (CHAT nhóm công ty kiểu Zalo + gắn phiếu — mục BF), **v108** (badge Đã giục đỏ cỡ 12px — mục BE), **v106** (Hỏi–Đáp trên phiếu + bảng nhắc khi mở app — mục BD), **v105** (ấn đúp phiếu cũ nhảy đúng đề xuất — mục BC), **v104** (bấm ô thống kê ra đúng danh sách — mục BB), **v103** (tab Lương PA4: 4 tab con + tìm kiếm + A→Z + Thẻ/Bảng — mục BA), **v102** (ứng lương xếp thuần theo thời gian — mục AZ), **v101** (mục AY — đã thay), **v100** (sửa Chrome autofill email vào ô tìm kiếm — mục AX), **v99** (nút xóa tài khoản trong Cài đặt — mục AW), **v98** (tài khoản đã duyệt không kẹt màn OTP — mục AV), **v97** (chấm số kiểu "đã xem" — mục AT), **v96** (chấm số đỏ trên thanh điều hướng — mục AS), **v95** (đăng nhập chậm 50s + nút GẤP bắt buộc lý do — mục AR), **v94** (vá 2 lỗi Zalo — mục AQ), **v93** (tính năng GỬI ẢNH CHUYỂN TIỀN QUA ZALO — mục AP), **v92** (sửa lưu Ý KIẾN D/H trên phiếu ứng lương — mục AO), **v91** (màn "chờ duyệt" treo + nối lại listener — mục AN), **v87** (ứng lương = phiếu chi kind='salary' — mục AM), **v85** (`APP_VERSION = '20260714-v85'`, `sw.js VERSION = '20260714-85'`, `version.txt = 20260714-v85`) — thêm **module Lương/ứng lương** (mục AL). Trước đó: luật v83+v84 (mục AJ, AK) + backup tự động. Ngày 14/07/2026.
...
### BR. 📊 v129–v130 — Ấn đúp bảng thống kê P.KH + hết toast 2 lần (03/08/2026)
Sau phát hành v128, D nghiệm thu tab Thống kê P.KH:
- **v129:** bảng "Theo công trình" — mỗi dòng `ondblclick="khStatsSiteGo(site)"` + `attachRowDoubleTap(el,'.kh-site-row')` (điện thoại) → nhảy sang tab Đề xuất, đặt sẵn bộ lọc công trình. Gắn touch 1 lần qua `el.dataset.dtap`.
- **v130 (user chốt "in đúc báo cáo kế toán"):** thêm **BẢNG CHI TIẾT TỪNG PHIẾU** dưới bảng công trình — cùng `class="excel-table"` + `report-row-link` + `ondblclick="jumpToKhProposal(id)"`, thanh `table-total-top` trên đầu; `attachKhReportDoubleTap` = bản sao y `attachReportDoubleTap` của kế toán, chỉ khác đích = `jumpToKhProposal`. Gắn 1 lần qua `el.dataset.dtap2`.
- **FIX "thông báo nhảy 2 lần":** bỏ `showToast` trong `khStatsSiteGo` — chạm kép trên điện thoại kích hoạt CẢ touchend LẪN dblclick → toast hiện 2 lần. Bảng kế toán nhảy IM LẶNG nên không dính; nay P.KH cũng im lặng → kép vô hại. **BẪY chung: đừng showToast trong handler ấn-đúp/chạm-2-lần.**
- Test preview: v129 8/8, v130 10/10 PASS. Deploy âm thầm → D duyệt → **phát hành v130** (gộp v129).

### BQ. 🏢 v124–v128 — Dự án "P.Kế hoạch" (PA2) + QUY TRÌNH XEM TRƯỚC (03/08/2026)
...
2. ✅ **Luật (deploy 03/08):** node `khProposals` (sao chép NGUYÊN mọi lớp bảo vệ của proposals, gate = approved && (D/H/Trang || dept kehoach/both)) + `khChat`/`khChatPin` (như groupChat/chatPin cùng gate) + `khChatRead` (self-only). `dept` ĐÃ KHÓA trong self-write userRoles (3 giá trị: ketoan mặc định/kehoach/both — user chốt thêm "Cả 2 phòng" cho người kiêm nhiệm; quyền duyệt vẫn theo role, dept chỉ quyết định NHÌN THẤY gì). Test 17/17 PASS bằng tài khoản test thật (file test_rules.js scratchpad): đăng ký cũ OK, kế toán cũ đọc OK, kế toán thuần KHÔNG vào được kho KH, P.KH đọc/ghi kho mới OK, TỰ ĐỔI PHÒNG BỊ CHẶN, both đọc cả 2. Có `firebase.json` mới để deploy rules bằng CLI.
3. ✅ **(v125)** Tab 📐 P.KH: phiếu kind='kh', **id tiền tố 'kh'** → `_propNodeFor/_imgNodeFor` tự định tuyến pushToFirebase/ảnh sang `khProposals`/`khImages`; listener kho kh (denied = coi như rỗng, không chặn app); lọc kh khỏi mọi view kế toán (matchFilter/getFilteredProposals/chuông/reconcile/_navPendingIds nhận false|true|'kh'); tab gồm DS thẻ (tái dùng renderCard đầy đủ duyệt/chuyển/GẤP/ảnh/hỏi-đáp) + Thống kê tháng/công trình + modal tạo (Chi phí chung/theo CT, `openKhModal`); edit-modal khóa người tạo, loại chi đổi được khi chưa duyệt; `jumpToKhProposal`; test 25/25.
4. ✅ **(v125)** Chat 2 kênh qua bộ helper `_c*` (`chatChannel` 'kt'|'kh'): mọi hàm chat chạy đúng kênh (gửi/xóa/ghim/trả lời/ảnh/gắn phiếu — gắn phiếu LỌC THEO KÊNH tránh lộ); badge tổng + badge từng kênh (`_chatSyncHeader`); kênh KH chỉ push D/H/Trang (không push 'other' tránh lộ nội dung); `_applyDeptView`: người kehoach thuần → nav rút gọn (P.KH/Thêm mới→modal KH/Vật tư/Cài đặt), trang kế toán tự lái về P.KH, chat vào thẳng kênh KH; test 21/21.
5. ✅ **(v125 + rules + Apps Script 30, deploy 03/08)** Siết đọc: dept='kehoach' BỊ CHẶN proposals/images/employees/salaryAdvances/groupChat/chatPin (thêm `dept !== 'kehoach'` vào 12 gate); VẪN đọc meta + plans; node `khImages` mới; test tài khoản thật **23/23 PASS** (kế toán cũ y nguyên, kín 2 chiều, tự đổi phòng bị chặn, both đọc cả 2). Apps Script **phiên bản 30**: act `khchi`/`khchiDel` → sheet **"Chi Kế hoạch"** 15 cột tự tô đẹp (tím #7c3aed, sort id desc); nút "Đồng bộ P.KH → Sheet" trong Cài đặt (D). Smoke hồi quy 7/7. **App v125 đã live ÂM THẦM (version.txt còn v124) — CHỜ D NGHIỆM THU rồi mới bump version.txt + sw.js.**
...
**NGHIỆM THU + PHÁT HÀNH (03/08/2026):** D nghiệm thu chuỗi âm thầm v125→v128, phát hiện 3 lỗi đều đã sửa: **v126** (a) `_cardHash` thiếu desc/person/site → sửa phiếu mà thẻ hiện bản cũ (lỗi nền ảnh hưởng MỌI loại phiếu, thêm vào hash + xóa cache trong saveEditProposal); (b) tab P.KH đổi ô tìm kiếm → 2 ô lọc "Tất cả người/Tất cả CT" giống Báo cáo. **v127** `renderAll` quên vẽ lại page-kh → đứng tại tab P.KH sửa phiếu phải F5 mới thấy (thêm block re-render giống tab Lương — BẪY: thêm tab mới ĐỌC từ proposals thì PHẢI nối vào renderAll). **v128** thứ tự nav do user chốt: Danh sách·Thêm mới·Báo cáo·Vật tư·P.KH·Lương·Cài đặt (remap màu nth-child; Lương tình cờ giữ index 5 nên `querySelectorAll('.nav-item')[5]` không đổi). **ĐÃ PHÁT HÀNH CHÍNH THỨC v128** (version.txt + sw.js bump, banner báo cả công ty). Dự án P.KH HOÀN TẤT.
...
- **App:** `syncLuongToSheet(p)` + `syncLuongDeleteToSheet(id)`. MỌI sự kiện phiếu ứng lương (tạo `saveAdvFromModal`→syncToSheet / sửa / duyệt / chuyển / từ chối / xóa) **tự chuyển hướng** sang sheet Lương (các guard `kind==='salary'` trong 8 hàm sync đổi từ `return` → gọi syncLuong). Phiếu chi đường cũ không đổi. `syncDeleteToSheet` gửi lệnh xóa **CẢ 2 sheet** (lúc gọi phiếu đã bị gỡ khỏi bộ nhớ, không biết loại — sheet không có dòng thì server bỏ qua).
- **Cài đặt (chỉ D):** mục `admin-luongsheet-section` + nút "🔄 Đồng bộ Lương → Google Sheet" (`syncAllLuongToSheet` — giãn 400ms/phiếu) để đẩy lại toàn bộ lần đầu/khi thiếu dòng.
- Đã test preview 14 mục (tham số emp/pos/tháng/tiền/còn-lại đúng; sự kiện lương → action luong, không đụng action chi; phiếu chi đường cũ; xóa gửi cả 2; nút chỉ D thấy).

### BN. ✨ v117 — tin đang ghim đổi màu ĐỎ (03/08/2026)
User chốt: tin được ghim phải đỏ cho dễ nhận biết. Bong bóng tin ghim (`isPinned = _chatPin.k===m._k` trong renderChatMsgs): nền `#fdecea` + viền `#e24b4a` + nhãn "📌 Tin đã ghim" đỏ đậm. Ghim/bỏ ghim đổi màu NGAY (`chatMenuPin`/`unpinChatMsg` gọi thêm renderChatMsgs; listener `chatPin` vẽ lại tin khi chat đang mở — đồng bộ mọi máy). Đã test 6 mục.

### BM. 💬 v116 — menu tin nhắn kiểu Zalo + @Tất cả + ghim + trả lời trích dẫn (03/08/2026)
User gửi ảnh menu Zalo, muốn chat có các tính năng đó + tag @Tất cả.
- **@Tất cả**: chip đầu dãy gợi ý tag (`_chatTagHint`, màu cam), tô cam đậm trong tin (`_chatHighlightTags` xử lý trước danh sách tên).
- **Bấm vào 1 tin → menu `#chat-msg-menu`** (`openChatMsgMenu`): 💬 **Trả lời** (`chatMenuReply` → `_chatReplyTo` preview trên ô nhắn `#chat-reply-preview`; gửi kèm `msg.re={by,text≤120,k}`; bong bóng hiện khung trích dẫn `reBox`, bấm → `scrollToChatMsg(k)` cuộn + nháy vàng) / ↗️ **Chia sẻ** (navigator.share, fallback copy) / 📄 **Copy tin nhắn** / 📌 **Ghim** (node `duyetchi/chatPin` {k,by,text≤300,ts} — luật đã deploy: thành viên đã duyệt ghim/bỏ ghim; thanh cam `#chat-pin-bar` dưới header, listener trong `_attachGatedListeners`; ✕ bỏ ghim) / ℹ️ **Xem chi tiết** (alert người gửi/vai/giờ) / 🗑 **Xóa** (quyền v113; nút ẩn nếu không được phép).
- Bong bóng có `id="chatmsg-<k>"` + onclick mở menu; ảnh/thẻ phiếu/nút 🗑 trong tin `event.stopPropagation()` để không mở nhầm menu.
- ⚠️ CHƯA làm: "Đánh dấu tin nhắn" + "Chọn nhiều tin nhắn" (đã nói rõ với user — ít dùng cho nhóm 8 người).
- Đã test preview 20 mục — tất cả đạt.

### BL. 📷 v115 — ẢNH trong Chat nhóm (02/08/2026)
User chốt: chat gửi được ảnh, chuyển tiếp ảnh từ phiếu sang, và copy/xóa/chia sẻ như chế độ ảnh mọi nơi.
- **Gửi ảnh:** nút 📷 cạnh ô nhắn (`#chat-img-input` → `handleChatImg`) → `compressImage` → `uploadToStorage('chat/<ts>_<rand>.jpg')` → `_chatAttachImg` preview chờ gửi (✕ = `clearChatAttachImg`) → Gửi kèm `msg.img` (**chỉ lưu URL Storage** — không nhồi base64 vào groupChat vì limitToLast(200) sẽ kéo nặng).
- **Chuyển tiếp từ phiếu:** nút "📤 Chat" góc trái màn xem ảnh lớn (`#imgModalToChat` → `forwardModalImgToChat`) — ảnh URL dùng thẳng, ảnh base64 cũ tự upload Storage trước; đóng modal → mở Chat với ảnh gắn sẵn.
- **Trong bong bóng:** thumbnail ≤190px (`chat-img-el`), bấm → `openChatImg` mở modal ảnh sẵn có (`modalKind='chat'`, zoom/copy/tải như mọi nơi); **nhấn giữ** thumbnail → menu copy/tải (+ xóa nếu tin mình hoặc D — xóa = `deleteChatMsg` cả tin). Modal long-press wiring chặn `modalKind==='chat'` trả null (không gọi nhầm removeZaloImage(null)).
- Chống gửi đúp + ẩn trùng liền kề tính cả `img`. **Luật (đã deploy):** `msg.img` string ≤ 2000. Đã test preview 13 mục.

### BK. 🐞 v114 — 3 bảng mở rộng hết màn hình máy tính (02/08/2026)
User báo: trên máy tính bảng "Chưa chuyển" chỉ chiếm một góc trái. Nguyên nhân: 3 bảng dùng khuôn `excel-table-scroll` bị ghim inline `width:<tổng-cột>px` theo khổ điện thoại. Sửa cả 3 (Báo cáo "Chưa chuyển" `renderExcelTable` kind pending · Thống kê Kế hoạch `renderPlanStats` · chế độ Bảng của Lương `_renderSalaryTable`): đổi thành `width:100%;min-width:<tổng>px` — màn rộng giãn hết, màn hẹp giữ min-width và trượt ngang như cũ. Lưu ý test: các tab Báo cáo render qua `requestAnimationFrame` nên đo DOM phải gọi thẳng `renderReportTables()`. Đã test 1280px: cả 3 bảng full 1260/1260, bóp 360px vẫn scroll.

### BJ. ✨ v113 — xóa/thu hồi tin nhắn chat nhóm (02/08/2026)
User chốt: D xóa được MỌI tin (kể cả xóa sạch); người khác xóa/thu hồi tin CHÍNH MÌNH. **Luật (đã deploy):** `groupChat` cấp cha `.write` cho role dung (xóa bất kỳ + xóa sạch cả node); `$id` cho người đã duyệt THÊM MỚI hoặc XÓA tin có `uid === auth.uid`; SỬA tin vẫn cấm. **UI:** nút 🗑 cạnh giờ trên từng bong bóng (`deleteChatMsg` — D thấy ở mọi tin, người khác chỉ tin mình, tin local chưa có `_k` chưa hiện nút; confirm trước xóa, ẩn ngay + server đồng bộ); header chat thêm **"🗑 Xóa tất cả"** `#chat-clear-btn` (chỉ D — toggle trong openChatPage; confirm 2 lần; `clearAllChat` remove cả node). Đã test 9 mục.

### BI. ✨ v112 — bảng nhắc tin nhắn có 2 nút "Để sau / Xong" (01/08/2026)
User chốt: bảng nhắc 💬 khi mở app có 2 nút. **"Để sau"** = đóng, lần mở tới nhắc lại (như cũ). **"Xong"** (`chatRemindDone`) = đánh dấu ĐÃ XEM tất cả tin trong bảng (ghi `chatSeen[uid]` từng phiếu, 1 lần `save()`, đồng bộ mọi thiết bị) → thôi nhắc + cập nhật chuông. Đã test 7 mục.

### BH. 🐞+✨ v111 — 4 chỉnh chat theo user (01/08/2026)
1. **Tin trùng 2 lần trong chat nhóm**: do bấm Gửi 2 lần lúc mạng chậm. Sửa kép: (a) `sendGroupMsg` **chặn gửi đúp** — tin giống hệt (text+ref) của mình trong 5s bị bỏ qua; (b) `renderChatMsgs` **ẩn tin trùng liền kề** (cùng người + cùng text + cùng ref, cách <60s) — che luôn cặp tin trùng đã lỡ nằm trên server (groupChat bất biến không xoá được).
2. Đổi tên nhóm chat → **"Nhóm Duyệt Chi"**.
3. Mũi tên quay lại (←): nền TRẮNG, chữ xanh đậm 22px + bóng — rõ nét.
4. Nút **"💬 Chat"** dời lên vị trí user khoanh (đứng TRƯỚC nút 🔄 trên header), nền TRẮNG chữ xanh đậm.
Đã test preview 8 mục.

### BG. 🐞 v110 — hết trùng 2 lần nội dung ý kiến D/H trên thẻ (01/08/2026)
Cầu nối v106 làm ý kiến D/H hiện 2 lần: dòng "Ý kiến D:" cũ + bong bóng hỏi–đáp cùng nội dung. Sửa `_buildCard`: nếu text ý kiến ĐÃ có trong `p.chat` (cùng role + cùng nội dung trim) → ẩn dòng cũ (`_showOpD/_showOpH`); phiếu cổ chỉ có ý kiến vẫn hiện; ý kiến khác nội dung chat → hiện cả hai. Đã test 3 kịch bản.

### BF. 💬 v109 — CHAT NHÓM CÔNG TY (PA2: kiểu Zalo + gắn phiếu, 01/08/2026)
User bàn 3 PA (có mockup) → chốt **PA2** + vị trí **Cách B**: nút "💬 Chat" trên header cạnh chuông, kèm số tin chưa đọc. Mục đích: nhắn nội bộ ngay trong app thay vì chạy sang Zalo, và tin nhắn **gắn được phiếu**.
- **Màn chat** `#chat-page` (overlay toàn màn hình, flex-column): header xanh + nút ←; vùng tin `#chat-msgs` (bong bóng — mình xanh phải, người khác trắng trái, tên tô màu theo vai `_chatRoleClr`, giờ HH:MM từng tin, phân cách theo ngày); hàng gợi ý tag `#chat-tag-row`; preview phiếu gắn `#chat-attach-preview`; ô nhập + nút 📋 + Gửi.
- **@tag** (`_chatTagHint`/`_chatTagPick`): gõ '@' cuối ô → dãy chip tên (Dũng/Hiền/Trang + `persons` + `employees`, trừ mình) → bấm chèn; render tô xanh đậm (`_chatHighlightTags`, escape trước rồi split/join theo tên dài→ngắn). Mỗi tin gửi → `sendPushNotif` tới các role khác mình.
- **Chưa đọc**: đếm tin `ts > _chatReadTs` và không phải của mình → badge `#chat-badge`; mốc đã-đọc tại `duyetchi/chatRead/<uid>` (chỉ mình đọc/ghi) → **đồng bộ đa thiết bị**; `markGroupChatRead` khi mở màn/tin mới đến lúc đang mở.
- **Gắn phiếu**: 📋 → `#chat-pick-modal` (tìm theo nội dung/người, 30 phiếu mới nhất cả chi+ứng lương) → `_chatAttachRef` + preview (✕ bỏ) → gửi kèm `msg.ref`. Thẻ phiếu trong chat (`_chatRefChip`) đọc **trạng thái sống** (`_chatRefStatus`), bấm → `chatOpenRef` đóng chat + jumpToProposal/jumpToSalaryAdvance. Đường tắt: nút **📤** trên mỗi thẻ phiếu (`openChatWithRef`).
- **Dữ liệu/luật (đã deploy)**: `duyetchi/groupChat/$id` — chỉ tài khoản đã duyệt, **append-only bất biến** (`!data.exists()`), text ≤ 2000; `chatRead/$uid` self-only isNumber. Client: listener `limitToLast(200)` qua biến `_gcQuery` (off đúng query) + `_crQuery` — nằm TRONG `_attachGatedListeners` → tự nối lại khi đăng nhập. Gửi = push + echo local (snapshot server thay thế nguyên mảng nên không trùng).
- Đã test preview 26 mục (badge đếm đúng — tin mình không đếm; tag; gắn/gửi kèm ref + push-queue; thẻ sống; 📤; đóng/mở). Không đụng luồng duyệt/chuyển.

### BE. ✨ v107+v108 — badge "Đã giục N lần" màu đỏ, cỡ 12px (31/07/2026)
v107 user yêu cầu to gấp đôi (24px) + đỏ; xem xong chê "thô, không đẹp" → v108 giảm về **12px**, GIỮ màu đỏ (`#dc0000` đậm 800, nền `#fdecea`; dòng giờ giục cũng đỏ). Vị trí: `_buildCard` khối `remindCount`. Bài học: cỡ chữ badge trong cột phải hẹp — 12px là chuẩn của mọi badge.

### BD. 💬 v106 — HỎI–ĐÁP trên phiếu + bảng nhắc khi mở app (31/07/2026)
User chốt (có mockup 3 màn đã duyệt): D/H ghi ý kiến → NGƯỜI ĐỀ XUẤT mở app thấy ngay 1 bảng giữa màn hình nội dung hỏi → trả lời ngay trên phiếu → D/H mở app thấy bảng trả lời. 2 điểm chốt kèm: ý kiến D/H trong nút Sửa cũng đổ vào Hỏi–Đáp; từng câu BẤT BIẾN.
- **Dữ liệu:** `p.chat = [{by, role, text, ts}]` (append-only) + `p.chatSeen = {uid: ts}` (đã-xem từng người, đồng bộ đa thiết bị). **Luật (đã deploy):** `chat/$i` bất biến sau gửi (so text/ts/by), text ≤ 2000; `chatSeen/$u` isNumber. KHÔNG có parent-clause chống xoá node chat (tránh kẹt sync client cũ — cùng tiền lệ opinionD/H).
- **Thẻ phiếu (`_buildCard`):** khung bong bóng chat (mình = xám phải, người khác = xanh trái, tên+giờ; màu tên theo vai D xanh/H tím/T cam) + ô trả lời tại chỗ `chat-inp-<id>` cho người liên quan; chưa có tin → nút nhỏ "💬 Hỏi–đáp về phiếu này" (`openChatInput`). `renderAll` HOÃN khi đang gõ ô chat (activeElement chat-inp-*) — flush khi blur (`_chatInpBlur`). `_cardHash` thêm chat.length + last ts.
- **`sendChatMsg`:** push entry + tự đánh dấu mình đã xem + render ngay + push notif cho các vai liên quan (từ opinions + chat roles, trừ mình; rỗng thì mặc định dung+hien). `saveEditProposal`: ý kiến D/H mới/đổi → append chat + push.
- **Bảng nhắc (`maybeShowChatReminder` / `#chat-remind-modal`):** gọi 1100ms sau mở app (2 chỗ: wasFirst + setRole — cùng chỗ maybeShowTransferReminder), chờ bảng nhắc chuyển của Trang đóng mới hiện. "Liên quan" = người đề xuất (`person`; ứng lương = `createdBy`) / D–H có ý kiến hoặc đã nhắn / T đã nhắn. Chưa đọc = entry `by≠mình && ts>chatSeen[uid]`. Bấm dòng → `chatRemindGo`: markSeen + nhảy phiếu (rẽ nhánh Lương). "Để sau" → lần mở tới nhắc lại.
- **Chuông:** `updateNotifBadge` cộng số tin chưa đọc; `renderNotifPanel` liệt kê mục 💬.
- Đã test preview 22 mục đúng kịch bản thật (D hỏi qua Sửa → Trang nhắc + trả lời trên thẻ → D nhắc lại → xem xong hết nhắc; người ngoài không thấy; chuông đếm đúng).

### BC. 🐞 v105 — ấn đúp phiếu CŨ không nhảy sang đề xuất (25/07/2026)
User báo: món đề xuất cũ ấn đúp (từ Báo cáo/thống kê) không nhảy. Nguyên nhân: trang chủ chỉ vẽ `_renderLimit` (150) phiếu MỚI nhất → phiếu cũ không có thẻ trong DOM → `jumpToProposal` tìm `card-<id>` không thấy, lặng lẽ return. **Sửa `jumpToProposal`:** ① tìm vị trí phiếu theo ĐÚNG thứ tự renderProposals (`max(ts,lastRemindAt)` desc, filter 'all'), nằm ngoài giới hạn thì `_renderLimit = idx+20`; ② xoá ô tìm kiếm (đang có chữ thì phiếu bị lọc mất); ③ cuộn sau 250ms + thử lại 700ms (vẽ nhiều thẻ chậm). **`closeNotifAndGo`** (chuông) viết lại: dùng chung jumpToProposal; phiếu `kind='salary'` rẽ sang tab Lương (`jumpToSalaryAdvance`). Đã test 300 phiếu: tái hiện đúng lỗi trước sửa (card thiếu), sau sửa thẻ tồn tại + cuộn được (300 thẻ vẽ), ô tìm xoá, chuông→ứng lương đúng nhánh.

### BB. ✨ v104 — bấm Ô THỐNG KÊ là ra đúng DANH SÁCH của nó (25/07/2026, áp toàn app)
User chốt: "tất cả những thống kê này khi ấn vào phải chỉ ra danh sách tương ứng — áp dụng cho toàn bộ app".
- **Tab Lương (TK nhóm + TK theo tên):** 3 ô Đã duyệt / Đã chuyển / Chưa chuyển → bấm mở **modal `#sal-stat-modal`** (`showSalaryStatList(which,'nhom'|'ten')`) liệt kê từng khoản (tên+chức vụ+nhóm+tháng+số tiền đúng loại: off / transferred / còn lại), bấm khoản → `jumpToSalaryAdvance` mở phiếu. Phạm vi = đúng bộ lọc kỳ/nhóm/ô tìm đang chọn (biến `_salStatsScope` set trong renderSalaryStats, `_salSnScope` trong renderSalaryStatsByName).
- **Báo cáo Tóm tắt:** 4 thẻ → TỔNG ĐỀ XUẤT chuyển sang tab Danh sách (`setReportTab('list',...)`); ĐÃ CHUYỂN / DT·CHỜ CK / CHỜ DUYỆT cuộn tới đúng bảng cùng trang (`_rptScrollTo`, viền cam 1.6s; 4 bảng makeTable bọc mỏ neo `rpt-sec-pend/appd/done/rej`).
- **Trang chủ:** 7 ô vốn đã bấm được (setFilter) — giữ nguyên.
- Đã test preview 11 mục (đếm 3/2/2 đúng từng loại, số còn-lại đúng, bấm khoản về thẻ, phạm vi theo ô tìm, 4 mỏ neo + cuộn OK).

### BA. ✨ v103 — tab Lương theo PA4: 4 tab con + tìm kiếm + A→Z + Thẻ/Bảng (23/07/2026)
User duyệt PA4 (đã bàn 4 phương án + xem mockup đủ màn) rồi chốt tách Thống kê làm 2. Kết quả **4 tab con** (`setLuongTab`: adv/emp/stats/statsName):
- **💵 Ứng lương:** ô tìm theo tên NV (`#sal-adv-search`, autocomplete off) + chọn xếp (`#sal-adv-sort`: time = mới nhất trên cùng v102 | name = A→Z localeCompare 'vi') + **nút gạt 🗂 Thẻ / 📊 Bảng** (`salSetView`, nhớ theo máy `dc_sal_view`, `_salSyncViewBtns` đồng bộ màu nút). Chế độ Bảng = `_renderSalaryTable`: excel-table (# · NV · Chức vụ · Nhóm · Đề xuất · D · H · Đã CK · Trạng thái `_salStatusLabel`) + dòng Tổng; chạm 2 lần dòng → `jumpToSalaryAdvance` (đã sửa: tự về chế độ Thẻ + xoá ô lọc trước khi cuộn tới thẻ).
- **👥 DS nhân viên:** ô tìm (`#sal-emp-search`, khớp tên/chức vụ) + xếp A→Z trong từng nhóm.
- **📊 TK nhóm:** giữ 3 thẻ tổng + 💸 Đã chuyển / ⏳ Chưa chuyển + Theo nhóm; BỎ mục theo tên (dời sang tab mới).
- **👤 TK theo tên** (`renderSalaryStatsByName`, `#sal-sn-*`): lũy kế từng người (duyệt/CK/còn + số khoản + chức vụ + nhóm), lọc kỳ (năm/tháng), ô tìm, xếp Ứng nhiều→ít / A→Z; **chạm 2 lần vào người → `salNameJump`** mở Ứng lương với ô tìm điền sẵn tên (helper double-tap chung `attachRowDoubleTap(container, selector)` — gọi thẳng `row.ondblclick`).
- Đã test preview 23 mục (4 tab, tìm/xếp/2 chế độ/đúp dòng/đúp người/TK nhóm sạch mục cũ). Không đổi luật/dữ liệu — chỉ hiển thị.

### AZ. ✨ v102 — Ứng lương xếp THUẦN theo thời gian (21/07/2026, thay v101)
User chốt tiếp: "thứ tự ưu tiên thời gian mới nhất lên trên cùng rồi cũ dần mới dễ nhìn" → **bỏ hẳn tiêu đề nhóm** (v101 vẫn giữ nhóm nên thứ tự bị xen kẽ theo nhóm). `renderSalaryAdvances` giờ là 1 dãy phẳng sort `max(ts, lastRemindAt)` giảm dần. Nhóm 🏢/🏗 vẫn hiện trên từng thẻ (site trong proposal-meta); phiếu bị giục nổi lên đầu. Đã test 5 mục: thứ tự đúng xuyên nhóm, hết header nhóm, thẻ vẫn hiện nhóm, giục nhảy lên đầu.

### AY. ✨ v101 — bảng Ứng lương: đề xuất MỚI lên trên cùng (21/07/2026) — ⚠️ đã thay bằng AZ
User: "đề xuất mới phải lên trên cùng mới đúng". Trước: nhóm xếp cứng Văn phòng → từng công trình (theo `sites`), phiếu ứng mới thuộc công trình đứng dưới bị chìm sâu. Sửa `renderSalaryAdvances`: **nhóm nào có đề xuất mới nhất nhảy lên đầu** (sort nhóm theo `max(ts, lastRemindAt)` giảm dần), trong nhóm vẫn mới-nhất-trước → phiếu vừa tạo luôn là thẻ đầu tiên. Đã test 3 mục (thứ tự nhóm, thứ tự trong nhóm, thẻ mới nhất đứng đầu).

### AX. 🐞 v100 — danh sách trống trên máy tính Bảo: Chrome autofill email vào ô tìm kiếm (21/07/2026)
Bảo mở app trên Chrome máy tính: thống kê có số đầy đủ nhưng danh sách "Chưa có đề xuất nào" — vì Chrome **tự điền email đăng nhập vào ô tìm kiếm** (autofill sau login) → `renderProposals` lọc theo chuỗi email → 0 kết quả. Sửa 2 lớp: ① ô `#search-input` thêm `autocomplete="off" name="dc-search-noauto"`; ② phòng vệ trong `renderProposals`: nếu ô tìm kiếm chứa **đúng** email đang đăng nhập (không ai tìm phiếu bằng email) → coi là autofill, tự xoá. Đã test: autofill tự xoá + danh sách hiện đủ, từ khoá thường không bị xoá oan. Chữa cháy thủ công (bản cũ): xoá chữ trong ô tìm kiếm là danh sách hiện lại.

### AW. ✨ v99 — nút XÓA tài khoản trong Cài đặt (20/07/2026)
User muốn tự dọn tài khoản lạ ngay trong app. Trong danh sách "Duyệt tài khoản được xem dữ liệu" (chỉ Dũng, `renderApproveList`): thêm nút 🗑 trên từng tài khoản (trừ chính mình). `deleteUserAccount(uid)`: guard role dung + guard tự-xóa, confirm 2 lần (kèm tên+email đọc từ DB), xóa `userRoles/<uid>` + `pendingOtp/<uid>`. ⚠️ Vỏ Firebase Auth KHÔNG xóa được từ app (cần Admin SDK) — kẻ đó đăng nhập lại thì hồ sơ tự tạo lại nhưng CHƯA DUYỆT → luật chặn đọc/ghi; muốn xóa tận gốc vỏ Auth: Firebase Console → Authentication → Users. Luật cho phép sẵn (dung ghi mọi $uid trong userRoles; pendingOtp dung đọc/xóa). Đã test 9 mục bằng stub db.ref trong preview.

### AV. 🐞 v98 — Bảo không đăng nhập được: KẸT MÀN OTP TREO (20/07/2026)
Bảo (`thaibaoo1203@gmail.com`, uid `cotY...`, approved=true) không vào được app: đăng ký 04/07 **chưa nhập OTP** (mã đã push cho Dũng rồi thất lạc), Dũng duyệt tay sau đó → `pendingOtp/<uid>` **treo vĩnh viễn** → luồng đăng nhập (`onAuthStateChanged`) thấy pendingOtp là bắt "Nhập mã 6 số" TRƯỚC khi xét role → kẹt mãi. Hằng (`thhdcongty@...`, uid `ATpw...`) dính y hệt. Bảo bèn tự đăng ký tài khoản MỚI `congty439jsc@gmail.com` (uid `yE7P...`) hôm 20/07 → cũng kẹt OTP + chưa duyệt.
- **Dọn server:** xoá pendingOtp treo của Bảo cũ + Hằng + tk rác gnail đã xoá; xoá luôn pendingOtp + userRoles của tài khoản trùng `congty439jsc` (Auth account vẫn tồn tại nhưng vô hại — không duyệt thì không xem được gì). `pendingOtp` giờ RỖNG, userRoles còn đúng 8 tài khoản.
- **Vá gốc (v98, app3):** trong luồng đăng nhập, nếu `userRoles.approved===true` mà còn pendingOtp → **tự xoá OTP và cho vào luôn** (OTP chỉ dành cho người đăng ký mới chưa duyệt). Không bao giờ tái diễn với người đã duyệt.
- **Bảo dùng tài khoản CŨ** `thaibaoo1203@gmail.com`; quên mật khẩu thì bấm "Quên mật khẩu?" (reset giữ nguyên uid/quyền). KHÔNG dùng `congty439jsc` nữa.

### AU. 🔧 Sửa backup tự động bị chết từ 16/07 (19/07/2026)
Rà 3 ngày phát hiện task `DuyetChi-Backup` fail liên tục với mã 0xC000013A — log bắt được tiến trình bị **gửi Ctrl+C (^C) ngay khi mở** trong console do Task Scheduler tạo (chạy tay thì OK; thứ gì đó trên máy ngắt console của taskeng, xuất hiện sau 15/07). S4U cần admin (bị denied). **Giải pháp chạy được:** task gọi `wscript.exe //B C:\Users\Public\DuyetChi\backup-hidden.vbs` → chạy `backup-run.cmd` **ẩn cửa sổ** → `backup-call.ps1` (UTF8-BOM, gọi script thật ở đường dẫn có dấu) → `scripts/backup-db.ps1`. Log tại `C:\Users\Public\DuyetChi\backup-log.txt`. Đã test: LastResult=0, backup 19/07 tạo được (307 phiếu, đủ userRoles/employees). Thiếu backup 16–18/07 (máy tắt/fail — chấp nhận). Nếu sau này lại fail: đọc log trên; chạy tay `scripts/backup-db.ps1` vẫn luôn được.

### AT. ✨ v97 — chấm số kiểu "ĐÃ XEM" (19/07/2026, chỉnh theo user)
User: "khi đã lăn xem rồi thì phải hết đếm số... hiện tại xem rồi số vẫn còn". Đổi cơ chế v96: **vào trang nào** (Danh sách/Lương/Kế hoạch) là các món hiện có được ghi nhớ **đã xem** (`localStorage dc_nav_seen_<sec>_<uid>` — theo từng tài khoản trên máy) và chấm tắt ngay; rời trang vẫn 0; chỉ **món MỚI** phát sinh sau đó mới đếm tiếp. `updateNavBadges()`: trang đang mở → markSeen + tắt; trang khác → đếm `ids.filter(!seen)`. `showPage` gọi updateNavBadges (try/catch); đăng xuất dọn `dc_nav_seen_`. Món status='transferred' (H duyệt + T chuyển đủ) không bao giờ đếm. Đã test 7 kịch bản.

### AS. ✨ v96 — chấm số đỏ trên thanh điều hướng (19/07/2026)
User muốn "biết đang có duyệt chi chỗ nào". Thêm `.nav-badge` (chấm đỏ số, ẩn khi 0, 99+) trên 3 nút: **Danh sách** (phiếu CHI chờ mình xử lý), **Lương** (phiếu ỨNG LƯƠNG chờ mình), **Kế hoạch** (kế hoạch pending/ordered mà D/H chưa xác nhận). Đếm theo VAI TRÒ đang đăng nhập (`_navPendingCount(kindSalary)` — D: chờ D duyệt, H: chờ H, T: chờ chuyển; other=0; `_navPlanCount()` cho D/H). `updateNavBadges()` móc vào renderAll + listener plans (đều try/catch — module thêm mới, không đụng cũ). Đã test 4 vai trò: D 1/1/1, H 2/1/2, T 1/1/0, other 0/0/0.

### AR. 🐞+✨ v95 — đăng nhập chậm ~50s + nút GẤP bắt buộc lý do (19/07/2026)
1. **ĐĂNG NHẬP 50s (lỗi thật của D, gốc từ v91):** khi ĐĂNG XUẤT, máy chủ hủy cả 4 kênh dữ liệu gác quyền (meta/proposals/plans/employees). v91 chỉ nối lại proposals và theo vòng chờ (không đăng nhập → 30s/lượt) → D đăng nhập lại đợi ~30–60s; meta/plans/employees chết hẳn tới khi tải lại trang. **Sửa:** gom 4 kênh vào `_attachGatedListeners()` trong initFirebaseSync (mỗi kênh `off('value')` trước `on` → gọi lại bao nhiêu lần cũng không trùng), gán vào biến toàn cục `_dcReattachGated`, và `onAuthStateChanged` gọi nó **NGAY khi đăng nhập**. Error path proposals: chưa đăng nhập → im lặng chờ login; 3 email nhà → retry 3s×10; người thường → màn chờ duyệt + nối lại 30s. Kho ảnh `once('value')` lỗi → `_imagesLoadStarted=false` để lượt `_mergeAndRender` sau đăng nhập TỰ THỬ LẠI (trước bị đánh dấu "đã tải" với kho rỗng vĩnh viễn).
2. **NÚT GẤP PHẢI CÓ LÝ DO** (user: "các bạn dùng bừa bãi quá"): chọn 🔴 Gấp ở form Thêm mới/Sửa → mở modal `#urgent-reason-modal` bắt gõ lý do; Hủy/để trống → radio về Bình thường. Chốt chặn cuối `ensureUrgentReason(ctx, prio)` trong submitProposal + saveEditProposal (kiểm TRƯỚC khi ghi trường nào). Lưu `p.urgentReason` (trường tự do, cap 2000 ký tự trong luật — đã deploy); thẻ phiếu hiện khung đỏ "Lý do gấp: ..."; `_cardHash` có urgentReason. Gửi xong reset lý do + radio, phiếu sau phải gõ mới. Đổi về BT khi Sửa → lý do tự xoá.
- Đã test preview 18 mục (nối lại kênh không trùng, modal hủy/rỗng/ok, chốt chặn, thẻ hiện lý do, Sửa giữ/xoá lý do).
...
- **Người đề xuất** (phiếu chi = `person`; ứng lương = `createdBy` — hàm `_zaloProposerOf`): banner xanh `#zalo-p-banner` "🔔 Còn N món chưa gửi Zalo" (chỉ chính họ thấy; realtime + đa thiết bị tự nhiên vì cờ nằm trên proposals). Bấm → modal `#zalo-list-modal`: danh sách chờ (nội dung·công trình·số đã chuyển·ngày) + nút **📸 Gửi Zalo** (mở `openImg(id,'proof',0)` — ảnh lớn, nhấn giữ Copy), **✅ Đã gửi**, **🔕 Tắt thông báo** (confirm). Mục **"ĐÃ XỬ LÝ"** phân biệt ✅ đã gửi (kèm ngày) / 🔕 đã tắt + nút ↩ hoàn tác. Bấm tên món → nhảy tới thẻ phiếu (chi → home, lương → tab Lương).
- **Trang**: banner đỏ `#zalo-t-banner` "📷 Chưa upload ảnh chuyển tiền cho N món" (món có transfer>0 mà proofImages rỗng) → modal chế độ 't' có nút 📤 Up ảnh từng món (gọi `uploadProof` sẵn có); up xong tự mất.
- **Điều kiện vào danh sách** (user chốt): CỨ có bất kỳ lần chuyển nào là nhắc (kể cả chuyển 1 phần); chưa có ảnh thì nút hiện "⏳ Chờ ảnh". Ứng lương NẰM TRONG luồng.
- Đã test preview 14 mục: đếm đúng theo người (không thấy món người khác), sent/mute/undo + banner cập nhật, phân biệt đã gửi/đã tắt, banner T đúng, home/report/stats không đổi.

### AO. 🐞 v92 — Ý KIẾN D/H trên phiếu ứng lương không lưu được (16/07/2026)
Modal "Sửa đề xuất" nạp danh sách người/công trường của PHIẾU CHI → với phiếu `kind='salary'`: tên nhân viên không có trong `persons` (ô Người đề xuất TRỐNG), nhóm "Văn phòng" không có trong `sites` (ô Công trường nhảy sai) → bấm Lưu bị chặn ("chọn người đề xuất") hoặc vi phạm đóng băng-sau-duyệt → D/H không lưu được Ý KIẾN. **Sửa (openEditProposal + saveEditProposal):** phiếu salary → 2 ô Người/Công trường render đúng giá trị phiếu và `disabled` (muốn đổi tên/nhóm thì sửa ở DS nhân viên); khi lưu lấy thẳng `p.person`/`p.site` (không đọc select). Phiếu chi thường không đổi hành vi. Đã test: D ghi ý kiến trên phiếu ứng đã duyệt → lưu OK, nội dung giữ nguyên; modal phiếu chi không bị khóa lây.
...
- **Ứng lương** (`renderSalaryAdvances` theo `salaryMonth` YYYY-MM, `salaryMonthShift`): mỗi khoản = 1 đề xuất ứng của 1 nhân viên. Nhóm theo Văn phòng + từng công trình. Luồng: ai đã duyệt cũng **đề xuất** (`openAdvModal`/`saveAdvFromModal`) → **Dũng duyệt số** → **Hiền duyệt số** → **Trang chuyển** (`openSalAmount`/`confirmSalAmount`, modal `sal-amount-modal`). Số chính thức = H nếu có, không thì D (`salOfficialApproved`). Có Từ chối (`salReject`), sửa/xoá khi chưa duyệt.
- **Thống kê** (`renderSalaryStats`): lọc Cả năm/tháng + nhóm; tổng đã duyệt, còn chờ chuyển, theo nhóm, **lũy kế theo nhân viên** (phục vụ đối soát cuối năm — TỔNG LƯƠNG user tự làm kín NGOÀI app, app chỉ quản ứng).
- **KHÔNG giữ kín** (cả nhóm xem như phiếu chi). **Luật v85**: employees/salaryAdvances read+write theo `approved`; salaryAdvances mô phỏng proposals — `approvedD` chỉ Dũng, `approvedH` chỉ Hiền, `transferred` chỉ Trang; nội dung (empName/group/month/amount) ĐÓNG BĂNG sau khi có duyệt; cap số tiền [0;100 tỷ] + độ dài chữ. Ghi CẢ mảng (atomic) an toàn vì trường không đổi qua được nhờ `newData===data`.
- **Đã test** end-to-end bằng preview (js): render 3 tab, tổng tiền đúng (đề xuất 19tr/duyệt 10tr), luồng D 8tr→chặn vượt→H 7tr→Trang chuyển 7tr→"Đã chuyển" đúng. Cú pháp JS `node --check` sạch.
- Version bump: `APP_VERSION=20260714-v85`, `version.txt=20260714-v85`, `sw.js VERSION=20260714-85`.
